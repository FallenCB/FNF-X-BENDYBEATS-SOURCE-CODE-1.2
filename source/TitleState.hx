	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('BendyBeatsHits'), 0);
			}
		}
		...
		var bg:FlxSprite = new FlxSprite(-13, -20).loadGraphic(Paths.image('theinkdemonsdomain'));
		...
		logoBl.updateHitbox();
		logoBl.screenCenter(X); // CENTERED GO BRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
		...
		//add(gfDance)
		//gfDance.shader = swagShader.shader;*/ // NO GF TITLE SCREEN :)
		add(logoBl);
		...
		//titleText.screenCenter(X); // Should be actually alligned properly now...       SIKE
		add(titleText);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('BendyBeats'); // credTextShit.text += '\nFunkin';
